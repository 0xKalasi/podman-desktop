...
export class PluginSystem {
  ...
  protected sendToWebContents(channel: string, ...args: unknown[]): void {
    try {
      ...
      console.debug(`Unable to send '${channel}' event to the main window`, err);
    }
  }
    ...
    return {
      send: <K extends keyof ApiSenderChannelMap>(
        ...
        try {
          ...
          // A receive() listener threw synchronously. Do not let that propagate back into
          // whatever called send() (often a stream 'data' handler), as that could silently
          // and permanently kill upstream processing (e.g. the docker/podman event stream).
          console.debug(`Error while notifying internal listeners for event '${String(channel)}'`, err);
        }
      },
